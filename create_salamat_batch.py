...
def main():
    ...
    sa_file = 'credentials/google-service-account-hspatsea-ocean-viking.json'
    ...
    if 'family_connection_other' in df_to_send.columns:
        df_to_send['family_connection'] = np.where(
            df_to_send['family_connection'] == 'other',
            df_to_send['family_connection_other'],
            df_to_send['family_connection']
        )

        df_to_send = df_to_send.drop(columns=['family_connection_other'])
    ...
    # Save Salamat sheet
    df_to_send.to_excel(f'data/Salamat_r{rotation_no}_{datetime.date.today()}.xlsx', index=False)
